  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        ...
        const userInfo = await fetch(`https://www.googleapis.com/oauth2/v1/userinfo?access_token=${tokenResponse.access_token}`)
          .then(res => res.json());
        ...
        toastService.success("Google login successful! Welcome back.");
        navigate("/");
      } catch (error) {
        console.error('Google login error:', error);
        toastService.error(error.message || "Google login failed. Please try again.");
      } finally {
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Button
          ...
          variant="outline"
          size="lg"
          className="w-full border-2 border-gray-200 hover:border-gray-300 bg-white hover:bg-gray-50 text-gray-700 font-semibold py-3 px-6 rounded-lg shadow-sm hover:shadow-md transition-all duration-300 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
          onClick={googleLogin}
